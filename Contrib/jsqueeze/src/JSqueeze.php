...
class JSqueeze {
	...
	public function squeeze($code, $singleLine = TRUE, $keepImportantComments = TRUE, $specialVarRx = FALSE) {
		...
		// Cleanup memory
		$this->charFreq = array_fill(0, 256, 0);
		$this->strings = $this->closures = $this->argFreq = [];
		$this->str0 = $this->str1 = '';
	...
	protected function makeVars($closure, &$tree, $key) {
		...
		$closure = preg_replace_callback("'(?<=[\n\{\}])var [^\n\{\};]+(?:\nvar [^\n\{\};]+)+'", [$this, 'mergeVarDeclarations'], $closure);
	...
	protected function renameVars(&$tree, $root) {
		...
		$tree['code'] = preg_replace_callback("#[.,{ ]?(?:[gs]et )?(?<![a-zA-Z0-9_\$@]){$this->varRx}:?#", [$this, 'getNewName'], $tree['code']);
	...
	protected function getNextName(&$tree = [], &$counter = FALSE) {
		if (FALSE === $counter) {
