	...
	public function addJavaScript($content, array $configuration) {
		...
		if (!$jsonData['settings']['disable_for_this_language']) {
			...
			return '<script id="cookieOptinData" type="application/json">' . json_encode($jsonData) . '</script>
					<link rel="preload" as="script" href="' . $siteBaseUrl . $file . '?' . $cacheBuster . '" data-ignore="1" crossorigin="anonymous">
					<script src="' . $siteBaseUrl . $file . '?' . $cacheBuster . '" data-ignore="1" crossorigin="anonymous"></script>';
		}
	}
